...
with st.sidebar:
    ...
    with st.spinner("Loading initial data..."):
        ...
        # --- ADD TRADE ID COLUMN (assuming 'id' exists in DB, else use row number) ---
        if 'id' in final_df.columns:
            final_df['trade_id'] = pd.to_numeric(final_df['id'], errors='coerce').fillna(0).astype(int)
        else:
            final_df = final_df.reset_index(drop=True)
            final_df['trade_id'] = final_df.index + 1  # 1-based sequential
    ...
    st.markdown("### Trade ID Range (Optional)")
    use_id_filter = st.checkbox("Filter by Trade ID Range", value=False)
    if use_id_filter:
        min_trade_id = int(final_df['trade_id'].min())
        max_trade_id = int(final_df['trade_id'].max())
        from_id = st.number_input("From Trade ID", min_value=min_trade_id, max_value=max_trade_id, value=min_trade_id, step=1)
        to_id = st.number_input("To Trade ID", min_value=min_trade_id, max_value=max_trade_id, value=max_trade_id, step=1)
    else:
        from_id = None
        to_id = None
    
    st.markdown("---")
    st.caption(f"Total trades in DB: {len(final_df)}")
    if use_id_filter and from_id is not None and to_id is not None:
        # Pre-filter just to show count (will be overridden by final filter)
        temp_filtered = final_df[(final_df['trade_id'] >= from_id) & (final_df['trade_id'] <= to_id)]
        st.caption(f"Filtered trades: {len(temp_filtered)} (IDs {from_id}-{to_id})")
    else:
        st.caption(f"Date range: {min_date} to {max_date}")
...
# Apply trade ID filter if enabled
if use_id_filter and from_id is not None and to_id is not None:
    fd = fd[(fd['trade_id'] >= from_id) & (fd['trade_id'] <= to_id)]
...
st.markdown(f"*Analysis Period: {start_date} to {end_date}*")
if use_id_filter:
    st.caption(f"Trade ID range: {from_id} - {to_id}")
