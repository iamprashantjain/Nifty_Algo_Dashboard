...
# ======================== view all trade_ids minimum from 1 ========================
...
with st.sidebar:
    ...
    with st.spinner("Loading initial data..."):
        ...
        # REMOVED: final_df = final_df[final_df['instrument'] == 'NIFTY']  # <-- FIX: Allow all instruments
    ...
    trade_id_min = st.number_input(
        "From Trade ID",
        min_value=1,  # Allow starting from 1
        max_value=int(final_df['trade_id'].max()),
        value=int(final_df['trade_id'].min()),
        step=1
    )
