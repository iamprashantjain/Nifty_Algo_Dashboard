...
#         # Store original unfiltered data for comparison
#         original_df = final_df.copy()
        
#         # ============================================
#         # FILTER TOGGLES - ADDED FOR COMPARISON
#         # ============================================
#         st.markdown("---")
#         st.markdown("### 🔄 Filter Toggles")
#         st.markdown("*Toggle filters on/off to compare results*")
        
#         # Wed/Fri Skip Toggle
#         skip_wed_fri = st.checkbox("Skip Wednesday & Friday Trades", value=True, 
#                                   help="Remove trades on Wednesdays and Fridays")
        
#         # 3-Day Halt After 5 SL Streak Toggle
#         halt_after_5sl = st.checkbox("3-Day Halt After 5 SL Streak", value=True,
#                                     help="Halt trading for 3 days after 5 consecutive stoploss hits")
        
#         # 2 SL Per Day Toggle
#         max_2sl_per_day = st.checkbox("Max 2 Stoploss Per Day", value=True,
#                                      help="Remove trades after 2 stoploss hits in a single day")
        
#         # Instrument Filter Toggle
#         filter_instrument = st.checkbox("Filter by Instrument", value=True)
        
#         # Option Type Filter Toggle
#         filter_option_type = st.checkbox("Filter by Option Type", value=True)
        
#         st.markdown("---")
        
#         # Apply Wed/Fri skip if enabled
#         if skip_wed_fri:
#             final_df = final_df[~final_df['entry_time'].dt.weekday.isin([2, 4])]
        
#         # Apply 3-day halt after 5 SL if enabled
#         if halt_after_5sl:
#             final_df = final_df.sort_values(['entry_date', 'entry_time']).reset_index(drop=True)
...
#         # Apply Max 2 SL per day if enabled
#         if max_2sl_per_day:
#             # Remove rows after 2 stoploss hit
#             final_df = pd.concat([group if len(group[group['exit_reason'].str.contains('Stoploss Hit', case=False, na=False)]) < 2 else group.loc[group[group['exit_reason'].str.contains('Stoploss Hit', case=False, na=False)].head(2).index] for date, group in final_df.groupby('entry_date')]).sort_index()
...
#         # Apply instrument filter if enabled
#         if filter_instrument:
#             instruments = st.multiselect("Instrument", options=final_df['instrument'].unique(), default=final_df['instrument'].unique())
#         else:
#             instruments = final_df['instrument'].unique()
#             st.info(f"All Instruments: {', '.join(instruments)}")
        
#         # Apply option type filter if enabled
#         if filter_option_type:
#             option_types = st.multiselect("Option Type", options=['CE', 'PE'], default=['CE', 'PE'])
#         else:
#             option_types = ['CE', 'PE']
#             st.info(f"All Option Types: CE, PE")
        
#         # Apply instrument filter
#         final_df = final_df[final_df['instrument'].isin(instruments)]
        
#         # Apply option type filter
#         final_df = final_df[final_df['option_type'].isin(option_types)]
        
#         # Continue with date range and trade ID filters
#         if final_df.empty:
#             st.error("No trades after applying filters. Please adjust your filter settings.")
#             st.stop()
...
#         # --- Trade ID Range Filter ---
#         st.markdown("### Trade ID Range")
        
#         trade_id_min = st.number_input(
#             "From Trade ID",
#             min_value=1,
#             max_value=int(final_df['trade_id'].max()) if not final_df.empty else 1,
#             value=int(final_df['trade_id'].min()) if not final_df.empty else 1,
#             step=1
#         )

#         trade_id_max = st.number_input(
#             "To Trade ID",
#             min_value=int(final_df['trade_id'].min()) if not final_df.empty else 1,
#             max_value=int(final_df['trade_id'].max()) if not final_df.empty else 1,
#             value=int(final_df['trade_id'].max()) if not final_df.empty else 1,
#             step=1
#         )
...
#         active_filters = []
#         if skip_wed_fri:
#             active_filters.append("✅ Skip Wed/Fri")
#         else:
#             active_filters.append("❌ Skip Wed/Fri (OFF)")
            
#         if halt_after_5sl:
#             active_filters.append("✅ 3-Day Halt after 5 SL")
#         else:
#             active_filters.append("❌ 3-Day Halt after 5 SL (OFF)")
            
#         if max_2sl_per_day:
#             active_filters.append("✅ Max 2 SL per Day")
#         else:
#             active_filters.append("❌ Max 2 SL per Day (OFF)")
...
#         # Store filter state for comparison display
#         st.session_state['filter_state'] = {
#             'skip_wed_fri': skip_wed_fri,
#             'halt_after_5sl': halt_after_5sl,
#             'max_2sl_per_day': max_2sl_per_day,
#             'filter_instrument': filter_instrument,
#             'filter_option_type': filter_option_type
#         }
...
# # Apply date and trade ID filters
# mask = (
#     (final_df['entry_date'] >= start_date) &
#     (final_df['entry_date'] <= end_date) &
#     (final_df['trade_id'] >= trade_id_min) &
...
# # Display active filter status at top
# st.markdown("---")
# st.markdown("### 🎯 Active Filters")
# filter_status_cols = st.columns(4)
# filter_statuses = [
#     ("Skip Wed/Fri", st.session_state['filter_state']['skip_wed_fri']),
#     ("3-Day Halt after 5 SL", st.session_state['filter_state']['halt_after_5sl']),
#     ("Max 2 SL per Day", st.session_state['filter_state']['max_2sl_per_day']),
#     ("Instrument Filter", st.session_state['filter_state']['filter_instrument'])
# ]
# for i, (name, active) in enumerate(filter_statuses):
#     with filter_status_cols[i]:
#         if active:
#             st.success(f"✅ {name}")
#         else:
#             st.warning(f"⛔ {name}")
...
# # ============================================
# # COMPARISON SECTION - SHOW WITH/WITHOUT FILTERS
# # ============================================
# st.markdown("---")
# st.markdown("## 🔍 Filter Comparison Analysis")

# # Create a version without filters for comparison
# # Get unfiltered data with same date range
# unfiltered_mask = (
#     (original_df['entry_date'] >= start_date) &
#     (original_df['entry_date'] <= end_date) &
#     (original_df['trade_id'] >= trade_id_min) &
#     (original_df['trade_id'] <= trade_id_max)
# )
# unfiltered_df = original_df[unfiltered_mask].copy()

# if not unfiltered_df.empty:
#     # Calculate metrics for unfiltered data
#     unfiltered_wins = unfiltered_df[unfiltered_df['net_pnl'] > 0]['net_pnl']
#     unfiltered_losses = unfiltered_df[unfiltered_df['net_pnl'] < 0]['net_pnl']
#     unfiltered_win_rate = (len(unfiltered_wins) / len(unfiltered_df) * 100) if len(unfiltered_df) > 0 else 0
#     unfiltered_total_pnl = unfiltered_df['net_pnl'].sum()
#     unfiltered_avg_trade = unfiltered_df['net_pnl'].mean()
#     unfiltered_profit_factor = unfiltered_wins.sum() / abs(unfiltered_losses.sum()) if len(unfiltered_losses) > 0 else np.inf
    
#     # Create comparison dataframe
#     comparison_data = pd.DataFrame({
#         'Metric': ['Total Trades', 'Net P&L (₹)', 'Win Rate (%)', 'Avg Trade (₹)', 'Profit Factor', 'Max Win (₹)', 'Max Loss (₹)'],
#         'Without Filters': [
#             len(unfiltered_df),
#             f"₹{unfiltered_total_pnl:,.0f}",
#             f"{unfiltered_win_rate:.1f}%",
#             f"₹{unfiltered_avg_trade:,.0f}",
#             f"{unfiltered_profit_factor:.2f}",
#             f"₹{unfiltered_wins.max():,.0f}" if len(unfiltered_wins) > 0 else "N/A",
#             f"₹{unfiltered_losses.min():,.0f}" if len(unfiltered_losses) > 0 else "N/A"
#         ],
#         'With Filters': [
#             len(fd),
#             f"₹{fd['net_pnl'].sum():,.0f}",
#             f"{win_rate:.1f}%",
#             f"₹{fd['net_pnl'].mean():,.0f}",
#             f"{profit_factor:.2f}",
#             f"₹{wins.max():,.0f}" if len(wins) > 0 else "N/A",
#             f"₹{losses.min():,.0f}" if len(losses) > 0 else "N/A"
#         ],
#         'Difference': [
#             f"{len(fd) - len(unfiltered_df):+,d}",
#             f"₹{fd['net_pnl'].sum() - unfiltered_total_pnl:+,.0f}",
#             f"{(win_rate - unfiltered_win_rate):+.1f}%",
#             f"₹{fd['net_pnl'].mean() - unfiltered_avg_trade:+,.0f}",
#             f"{(profit_factor - unfiltered_profit_factor):+.2f}",
#             "N/A",
#             "N/A"
#         ]
#     })
    
#     st.markdown("### 📊 Filter Impact Comparison")
...
#             x=['Total P&L', 'Avg P&L'],
#             y=[unfiltered_total_pnl, unfiltered_avg_trade],
#             marker_color='#FF6B6B'
#         ))
#         fig_compare.add_trace(go.Bar(
#             name='With Filters',
#             x=['Total P&L', 'Avg P&L'],
#             y=[fd['net_pnl'].sum(), fd['net_pnl'].mean()],
#             marker_color='#4ECDC4'
#         ))
#         fig_compare.update_layout(
#             title="P&L Comparison: With vs Without Filters",
#             barmode='group',
#             template="plotly_white",
#             height=400,
...
#             y=[unfiltered_win_rate, unfiltered_profit_factor],
#             marker_color='#FF6B6B'
#         ))
#         fig_rate.add_trace(go.Bar(
#             name='With Filters',
#             x=['Win Rate', 'Profit Factor'],
#             y=[win_rate, profit_factor],
#             marker_color='#4ECDC4'
#         ))
...
#     # Additional comparison: Distribution overlay
#     st.markdown("### 📈 P&L Distribution Comparison")
#     fig_dist_compare = go.Figure()
    
#     # Histogram for unfiltered data
#     fig_dist_compare.add_trace(go.Histogram(
#         x=unfiltered_df['net_pnl'],
#         nbinsx=30,
#         name='Without Filters',
#         opacity=0.6,
#         marker_color='#FF6B6B'
...
#         title="P&L Distribution: With vs Without Filters",
#         barmode='overlay',
#         template="plotly_white",
#         height=400,
#         xaxis_title="P&L (₹)",
