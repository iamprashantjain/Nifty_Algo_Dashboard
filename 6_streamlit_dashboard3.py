...
st.markdown("---")
st.markdown("## 📅 Monthly Profit & Loss Calendar")

calendar_df = (
    fd.groupby("entry_date")
    .agg(
        Net_PnL=("net_pnl", "sum"),
...
calendar_df["entry_date"] = pd.to_datetime(calendar_df["entry_date"])
calendar_df["Year"] = calendar_df["entry_date"].dt.year
calendar_df["Month"] = calendar_df["entry_date"].dt.month
calendar_df["MonthName"] = calendar_df["entry_date"].dt.strftime("%B")
...
months = calendar_df.loc[
    calendar_df.Year == selected_year,
    "Month"
].unique()
...
for month in months:
    ...
    total_month_pnl = mdf.Net_PnL.sum()

    month_name = calendar.month_name[month]

    st.markdown(
        f"### {month_name} {selected_year} &nbsp;&nbsp;&nbsp; "
        f"{'🟢' if total_month_pnl>=0 else '🔴'} "
        f"₹{total_month_pnl:,.0f}"
    )
    ...
    for week in cal:
        ...
        for day in week:
            ...
            if day == 0:
                ...
                dt = pd.Timestamp(selected_year, month, day)

                rec = mdf[mdf.entry_date == dt]

                if len(rec):

                    pnl = rec.Net_PnL.iloc[0]
                    gross = rec.Gross_PnL.iloc[0]
                    charges = rec.Charges.iloc[0]
                    trades = rec.Trades.iloc[0]

                    row.append(pnl)

                    hover.append(
                        f"<b>{dt.strftime('%d %b %Y')}</b><br>"
                        f"Net P&L : ₹{pnl:,.0f}<br>"
                        f"Gross : ₹{gross:,.0f}<br>"
                        f"Charges : ₹{charges:,.0f}<br>"
                        f"Trades : {trades}"
                    )
                    ...
                    row.append(0)
                    hover.append(
                        f"{dt.strftime('%d %b %Y')}<br>No Trades"
                    )
    ...
    fig.add_trace(
        ...
        go.Heatmap(
            ...
            y=[f"Week {i+1}" for i in range(len(z))],
