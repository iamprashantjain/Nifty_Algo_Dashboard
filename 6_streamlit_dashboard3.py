...
def create_calendar_heatmap_exact(daily_data, year, month):
    """Create a calendar heatmap exactly like the image"""
    ...
    for week_idx, week in enumerate(cal_month):
        ...
        day_numbers.append(week_days)
    
    # Create the heatmap
    fig = go.Figure()
    
    # Custom color scale matching the image
    colorscale = [
        [0.0, '#8B0000'],      # Dark red for very negative
        [0.25, '#FF4444'],     # Red for negative
        [0.4, '#FF6B6B'],      # Light red
        [0.5, '#FFFFFF'],      # White for zero
        [0.6, '#90EE90'],      # Light green
        [0.75, '#32CD32'],     # Green for positive
        [1.0, '#006400']       # Dark green for very positive
    ]
    ...
    fig.add_trace(go.Heatmap(
        z=heatmap_data,
        x=['S', 'M', 'T', 'W', 'T', 'F', 'S'],
        y=['' for _ in range(len(heatmap_data))],  # Empty labels for rows
        colorscale=colorscale,
        zmid=0,
        showscale=False,  # Hide color scale to match image
        hovertemplate='Day: %{customdata}<br>P&L: ₹%{z:,.0f}<extra></extra>',
        customdata=day_numbers,
        text=day_numbers,
        texttemplate='%{text}',
    ...
    # Update layout to match image exactly
    fig.update_layout(
        height=350,
...
# ============================================
# CALENDAR HEATMAP SECTION - EXACTLY LIKE IMAGE
# ============================================
st.markdown("---")
...
if not fd.empty:
    ...
    with col1:
        if st.button("◀", key="prev_year_heatmap"):
            current_year = st.session_state.heatmap_year
    ...
    with col2:
        if st.button("◀", key="prev_month_heatmap"):
            current_year = st.session_state.heatmap_year
            current_month = st.session_state.heatmap_month
    ...
    with col3:
        ...
        display_text = f"{datetime(st.session_state.heatmap_year, st.session_state.heatmap_month, 1).strftime('%B %Y')}"
        st.markdown(f"<h3 style='text-align: center;'>{display_text}</h3>", unsafe_allow_html=True)
    
    with col4:
        if st.button("▶", key="next_month_heatmap"):
            current_year = st.session_state.heatmap_year
            current_month = st.session_state.heatmap_month
    ...
    with col5:
        if st.button("▶", key="next_year_heatmap"):
            current_year = st.session_state.heatmap_year
            if current_year + 1 in available_years:
                st.session_state.heatmap_year = current_year + 1
    ...
    # Month summary like in the image
    month_start = pd.Timestamp(f"{selected_year}-{selected_month:02d}-01")
    if selected_month == 12:
        month_end = pd.Timestamp(f"{selected_year+1}-01-01") - pd.Timedelta(days=1)
    else:
    ...
    if not month_data.empty:
        ...
        st.markdown("""
            <div style='display: flex; justify-content: center; align-items: center; gap: 20px; margin: 10px 0; padding: 10px; background-color: #f8f9fa; border-radius: 8px;'>
                <div style='display: flex; align-items: center; gap: 5px;'>
                    <div style='width: 20px; height: 20px; background: #8B0000; border-radius: 3px;'></div>
                    <span style='font-size: 12px;'>Loss</span>
                </div>
                <div style='display: flex; align-items: center; gap: 5px;'>
                    <div style='width: 20px; height: 20px; background: #FFFFFF; border: 1px solid #ddd; border-radius: 3px;'></div>
                    <span style='font-size: 12px;'>Neutral</span>
                </div>
                <div style='display: flex; align-items: center; gap: 5px;'>
                    <div style='width: 20px; height: 20px; background: #32CD32; border-radius: 3px;'></div>
        ...
        st.markdown(f"### {datetime(selected_year, selected_month, 1).strftime('%B %Y')} Summary")
        
        month_pnl = month_data['net_pnl'].sum()
        month_trades = len(month_data)
        month_win_rate = (month_data['net_pnl'] > 0).mean() * 100
        month_avg_pnl = month_data['net_pnl'].mean()
        month_max_win = month_data['net_pnl'].max() if len(month_data) > 0 else 0
        month_max_loss = month_data['net_pnl'].min() if len(month_data) > 0 else 0
        ...
        with col2:
            st.metric("Total Trades", month_trades)
        
        with col3:
            st.metric("Win Rate", f"{month_win_rate:.1f}%")
        
        with col4:
            st.metric("Avg P&L", f"₹{month_avg_pnl:,.0f}")
        
        with col5:
            st.metric("Best/Worst", f"₹{month_max_win:,.0f} / ₹{month_max_loss:,.0f}")
        ...
        with st.expander("📋 Daily Breakdown for Selected Month"):
            ...
            month_daily_summary.columns = ['Total P&L', 'Trades', 'Avg P&L', 'Win Rate']
            month_daily_summary['Win Rate'] = month_daily_summary['Win Rate'] * 100
            month_daily_summary = month_daily_summary.sort_index()
            
            # Format the data
            display_df = month_daily_summary.copy()
            display_df['Total P&L'] = display_df['Total P&L'].apply(lambda x: f"₹{x:,.0f}")
            display_df['Avg P&L'] = display_df['Avg P&L'].apply(lambda x: f"₹{x:,.0f}")
            display_df['Win Rate'] = display_df['Win Rate'].apply(lambda x: f"{x:.1f}%")
            
            st.dataframe(display_df, use_container_width=True)

else:
    st.warning("No data available to display calendar heatmap")
