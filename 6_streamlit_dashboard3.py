...
# =============== added fyers like monthly calendar ===================
...
def create_calendar_heatmap_plotly(fd):
    """
    Create a compact monthly P&L calendar matching the reference layout:
    - 6 months per row
    ...
    pnl_dict = {
        row['entry_date'].date(): row['net_pnl']
    ...
    # April -> March
    # ---------------------------------------------------------
    min_date = min(pnl_dict.keys())
    max_date = max(pnl_dict.keys())
    ...
    # ---------------------------------------------------------
    non_zero_pnls = [
        pnl for pnl in pnl_dict.values()
        if pnl != 0
    ]

    max_abs = max(
        [abs(x) for x in non_zero_pnls],
        default=1
    )
    ...
    # COLOR FUNCTION
    # ---------------------------------------------------------
    def get_cell_color(pnl):
        if pnl is None or pnl == 0:
            return "#2d2d2d"

        intensity = min(abs(pnl) / max_abs, 1.0)

        # Keep cells readable while creating intensity
        if pnl > 0:
            # dark -> bright green
            r = int(35 - (35 * intensity))
            g = int(90 + (165 * intensity))
            b = int(55 - (55 * intensity))
            return f"rgb({r},{g},{b})"

        else:
            # dark -> bright red
            r = int(100 + (155 * intensity))
            g = int(45 - (35 * intensity))
            b = int(45 - (35 * intensity))
            return f"rgb({r},{g},{b})"
    ...
    def format_pnl(pnl):
        ...
        if abs(pnl) >= 1000:
            return f"{sign}₹{abs(pnl) / 1000:.1f}K"
        else:
        ...
        for week in weeks:
            ...
            for day in week:
                ...
                if day == 0:
                    ...
                    pnl = pnl_dict.get(date_obj, None)

                    if pnl is None or pnl == 0:

                        # Normal non-trading day
                        html += f"""
                        <div
                            class="pnl-day no-trade"
                            title="{date_obj.strftime('%d %b %Y')} — No trades"
                        >
                        ...
                        bg = get_cell_color(pnl)
                        pnl_text = format_pnl(pnl)

                        html += f"""
                        <div
                            class="pnl-day traded"
                            style="background:{bg};"
                            title="{date_obj.strftime('%d %b %Y')} — P&L: {pnl_text}"
                        >
                            <span class="day-number">{day}</span>
                        </div>
                        """
    ...
    for year, month in months:
        months_html += create_month(year, month)
    ...
    <div class="pnl-calendar-wrapper">
